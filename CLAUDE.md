...
`.github/workflows/daily-signal.yml` runs `daily_report.py` at `30 22 * * 1-5` UTC — after the U.S. close, ~1.5h before the JP open. Scheduled runs pass `--require-live --skip-if-stale 0` so a synthetic or stale book is never sent; `workflow_dispatch` exposes `date` / `dry_run` / `offline` / `channels` inputs. Channel selection outside manual overrides comes from the `REPORT_CHANNELS` repo variable. Rendered output is always uploaded as an artifact. Note GitHub disables cron workflows after 60 days of repo inactivity.
