...
> **注意（論文のギャップ）**: 上の右辺の等号（`Σ_JU Σ_UU^{-1} = 1/(1+σ_U²)·V*_J V*_U^T`）は、
> 命題2の証明の Woodbury の段で `V*_U^T V*_U = I_K`（V*_U の列直交性）を使って初めて成立する。
> ところが `V^(K)_t` は**同時PCAの固有ベクトル**であり、その U ブロックは正規直交にならない。
> 論文自身の事前方向 V0 で測ると U ブロックの Gram は
>
> ```
> [[0.393, 0.488, 0    ],
>  [0.488, 0.607, 0    ],
...
> ```
>
> で、グローバル因子と日米スプレッド因子が強く相関する。この K×K のゆがみは
> `ẑ_J` の**銘柄間ランキングを変える**ため、単調変換として吸収されない。
> 本実装の既定は論文どおり `B = V_J V_U^T` のままだが、命題2の一般形
> `B = V_J (V_U^T V_U + σ² I_K)^{-1} V_U^T` を `--predictor ridge` で選べる
> （`common.predictor_matrix`）。σ² は保持した固有値の割合から推定する。

> **注意（もう一つの暗黙の仮定）**: 式(23)(24) は `z_{U,t}` と `z_{J,t+1}` の関係しか
> 規定しないが、実装は `C_t` を**同時点**の日米 rcc から推定する。日米が同日に
> 共変動しなければ `V*_J` はそもそも同時相関から識別できない。この仮定は論文本文に
> なく、`verify_logic.py --contemp-rho` で強さを変えて影響を確認できる。
...
> **論文の誤植2点。いずれも本実装の側が正しい。**
> - 式(27)(28) の年率化係数は 12（月次の慣行）だが、対象は日次 Open-to-Close なので
>   252 が正しい。表2の数値（AR 23.79% / RISK 10.70%）とも 252 でしか整合しない。
>   本実装は `periods_per_year` を引数化し既定 252 とした。
> - 式(30) の MDD は `max_{τ∈{1..T}} W_τ`（全期間の最大）と書かれているが、これでは
>   ピーク到達**前**の下落まで拾ってしまう。上式のとおりランニングピーク `max_{τ≤t}`
>   が正しく、`performance_metrics` は `np.maximum.accumulate` で実装している。
...
**評価期間について。** 論文4.1節はデータ期間を 2010年1月〜2025年12月と書いているが、
表1の観測数は通期上場銘柄が 2590、XLRE が 2409、XLC が 1758 である。差分の 181 日・832 日は
それぞれ 2015-01-01 から XLRE（2015-10-08）・XLC（2018-06-19）の上場日までの共通営業日数に
一致する。したがって **C_full の推定が 2010–2014、評価（表1・表2）が 2015-01 以降**で、
両者は重なっていない。`backtest_run.py` はこの分割を既定とし、`--eval-start` が
事前期間と重なる場合は警告する。
...
| `backtest_run.py` | **実証バックテスト**: 実価格で表1（銘柄別統計）・表2（AR/RISK/R/R/MDD）を再現。既定は事前 2010–2014 / 評価 2015- の分割。合成フォールバックでの実行は `--offline` を明示しない限り拒否する |
...
- **式(25)の前提の検証**: 生成過程が `V*_U^T V*_U = I_K` を満たすことを確認したうえで、
  論文の閉形式 `1/(1+σ_U²)·V*_J V*_U^T` が定義から計算した BLP `Σ_JU Σ_UU^{-1}` と
  一致するか（`cos` と ノルム比）を表示する。前提が崩れていれば警告を出す
- **命題1**: `rank(B*) ≤ K` の確認
- **正則化の効果**: ウィンドウ長 `L` を変えたときの `B*` への近似精度。行列そのものの
  コサインと、**その予測子が実際に作る建玉の順位相関（rank IC）** の両方を出す。
  戦略は `B z_U` を並べて上下 q を取るだけなので、後者が本来見るべき量である
- **表2の再現**: MOM / PCA_PLAIN / PCA_SUB / DOUBLE の AR・RISK・R/R・MDD

#### 生成過程のノブ（検証が失敗しうること）

以前の生成過程は真のローディングが事前方向そのものだったため、正則化が負けようが
なかった。現在は以下の3つを明示的なノブにしてあり、いずれも既定値は有利な端ではない。
十分に振れば PCA_PLAIN が勝つ——その条件下ではそれが正しい答えである。

```bash
python verify_logic.py --prior-misspec 1.0   # 事前分布が真の構造と無関係な場合
python verify_logic.py --contemp-rho 0.0     # 日米の同日共変動を切る（V*_J が識別不能に）
python verify_logic.py --loading-drift 0     # 因子構造を固定（短ウィンドウでも困らなくなる）
python verify_logic.py --predictor ridge     # 式(21) ではなく命題2の一般形を使う
```

- `--prior-misspec`: 事前方向とランダム部分空間の混合比（0 = 事前が真、1 = 無関係）
- `--contemp-rho`: 同時点の日米共変動の強さ。論文本文にない仮定（1.8節の注意を参照）
- `--loading-drift`: 真のローディングの平均回帰型のゆらぎ。固定（0）にすると L=60 でも
  素のPCAで十分正確になり、正則化はバイアスを足すだけになる。論文が主張する機構
  （短いローリングウィンドウでは構造が不安定）を生成過程に入れるためのノブ

図（6パネル）: 真の `B*`／正則化PCAの `B^(K)`／素のPCAの `B^(K)`
のヒートマップ比較、ウィンドウ長 vs 近似精度（コサインと rank IC）、累積リターン、R/R 棒グラフ。

### 4.1b 実証バックテスト（表1・表2）

```bash
python backtest_run.py                          # 論文の設定（事前 2010-2014 / 評価 2015-）
python backtest_run.py --eval-start 2018-07-01  # XLC 上場後に限定
python backtest_run.py --predictor ridge        # 命題2の補正版
python backtest_run.py --offline                # 合成データで配線確認（数値に意味はない）
```

`output/backtest_cumulative.png` と `output/backtest_returns.csv` を出力する。
ライブ取得に失敗して合成データにフォールバックした場合、`--offline` を明示していなければ
**実行を拒否する**（合成の数値を表2の再現と取り違えないため）。

表3・表4（Fama–French 3 / Carhart 4 回帰）は未実装。日本側のファクターリターン系列を
本リポジトリでは取得していないため。
...
```
[Premise] Prop. 2 assumes V*_U^T V*_U = I_K (used in the Woodbury step):
    max |V*_U^T V*_U - I| : 2.22e-16
    eq.(25) vs Sigma_JU Sigma_UU^-1 : cos=1.000000  norm ratio=1.000000
    -> eq. (25) holds exactly here; using it as the target.

[Prop. 2 correction] alignment to B* at L=60, lambda=0.9:
    predictor=paper  : +0.115     ← 式(21) そのまま
    predictor=ridge  : +0.247     ← 命題2の一般形（1.8節の注意）

[Claim 2] Mean cosine similarity of B^(K) to B* vs window length:
  cosine = similarity of the whole matrix to B*;  IC = rank correlation of the book they imply
  L  plain    sub plain_IC sub_IC
 20 +0.383 +0.098   +0.074 +0.315
 60 +0.499 +0.115   +0.139 +0.335
180 +0.636 +0.103   +0.242 +0.346

[Claim 3] Long/short performance on synthetic data (annualised, 252d):
             AR  RISK    RR    MDD
MOM       -5.37 11.63 -0.46 -32.73
PCA_PLAIN  1.56 12.26  0.13 -27.21
PCA_SUB   12.67 11.92  1.06 -11.37     ← R/R 最良・MDD 最小（論文の結論と整合）
DOUBLE     1.17 14.26  0.08 -26.37
```

読み方:

- 式(25) は**この生成過程では厳密に成立する**（cos=1.000000）。これは前提を満たすように
  生成過程を組んだうえでの確認であり、前提が崩れれば警告が出る。
- 行列のコサインでは PCA_SUB が PCA_PLAIN に**負けている**（0.115 vs 0.499）のに、
  rank IC では大きく勝つ（0.335 vs 0.139）。縮小推定は成分ごとのバイアスと引き換えに
  分散を落とすため、行列そのものの距離では不利に出る。一方、式(3)–(7) は17銘柄の
  **順位しか使わない**ので、そのバイアスはほとんど無害である。R/R の差はここから来る。
- 実数値は乱数シードに依存する。seed 0〜4 では PCA_SUB が一貫して R/R 最良。
...
- **因子スコアのラベル**: `C^reg_t` の固有ベクトルは事前方向 v1/v2/v3 そのものではなく、
  固有値の順が事前方向の順と一致する保証もない。実データでは上位2本は両者の
  混合として出る（各事前方向との |cos| が 0.75〜0.80 程度で、1.0 にならない）。
  そのため `f_1 = global` のような位置決め打ちのラベルはやめ、
  `realtime_run.factor_labels` が |cos| 最大の事前方向名と、その |cos| 値を併記する。
  さらに固有ベクトルの符号は任意なので、**`f_k` の符号自体には意味がない**
  （`B = V_J V_U^T` は列の符号反転に対して不変で、意味を持つのは `ẑ_J` のみ）。
- **欠測の扱いの統一**: 「日本側は全銘柄の窓が揃っていることを要求し、米国側は
  窓の揃った銘柄だけでその日の同時PCAを回す」というルールを
  `common.select_subuniverse` に切り出し、`snapshot_at` と `run_backtest` の双方で使う。
  以前 `run_backtest` は1銘柄でも欠測があるとその日を丸ごと捨てていたため、
  XLRE（2015-10上場）・XLC（2018-06上場）の影響で 2018年以前が全滅していた。
  なおシグナルは窓 `W_t` と米国の当日終値しか読まないので、日本側の**当日** rcc の
  有無は要求しない。
- **事前期間のリーク防止**: `build_prior` が C_full を推定した期間を返し、
  `snapshot_at` はシグナル日がその期間内にある場合に警告する
  （`--allow-in-sample` で抑止）。
...
- **表3・表4（Fama–French 3 / Carhart 4 回帰）は未実装。** 日本側のファクターリターン
  系列の調達が別課題のため。
- 式(21) `B = V_J V_U^T` が命題2の `B*` と一致するには `V_U^T V_U = I_K` が必要だが、
  同時PCAの U ブロックではこれが成立しない（1.8節の注意）。既定は論文忠実のままとし、
  補正版は `--predictor ridge` で選択する。
- 同時PCAから `V*_J` を推定するには「日米の同日共変動」が必要で、これは論文本文に
  記述のない仮定である（1.8節の注意）。
